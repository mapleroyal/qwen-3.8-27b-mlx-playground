...
const inputGroupAddonVariants = cva(
  "text-label-lg flex h-auto cursor-text items-center justify-center gap-2 py-2 text-muted-foreground select-none group-data-[disabled=true]/input-group:opacity-50 **:data-[slot=kbd]:rounded-4xl **:data-[slot=kbd]:bg-muted-foreground/10 **:data-[slot=kbd]:px-1.5 [&>svg:not([class*='size-'])]:size-4",
  {
    variants: {
      align: {
        "inline-start":
...
const inputGroupButtonVariants = cva(
  "text-label-lg flex items-center gap-2 rounded-4xl shadow-none",
  {
    variants: {
      size: {
...
function InputGroupText({ className, ...props }) {
  return (
    <span
      className={cn(
        "text-body-md flex items-center gap-2 text-muted-foreground [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4",
        className,
      )}
      {...props}
    />
